 ...
 *
 */
#ifndef GB_CORE_MATRIX_H
#define GB_CORE_MATRIX_H
 ...
 */
static __tb_inline__ tb_void_t gb_matrix_apply_point(gb_matrix_ref_t matrix, gb_point_ref_t point)
{
    // check
    tb_assert_abort(matrix && point);
 ...
 * @param line      the line
 */
static __tb_inline__ tb_void_t gb_matrix_apply_line(gb_matrix_ref_t matrix, gb_line_ref_t line)
{
    // check
    tb_assert_abort(matrix && line);
    
    // apply it
	gb_matrix_apply_point(matrix, &line->p0);

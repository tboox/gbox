 ...
 */
static tb_long_t gb_tessellator_active_region_comp(tb_item_func_t* func, tb_cpointer_t ldata, tb_cpointer_t rdata)
{
    // TODO
    return 0;
}
#ifdef __gb_debug__
static tb_char_t const* gb_tessellator_active_region_cstr(tb_item_func_t* func, tb_cpointer_t data, tb_char_t* cstr, tb_size_t maxn)
{
    // check
...
static tb_void_t gb_tessellator_active_regions_insert_done(gb_tessellator_impl_t* impl, tb_size_t tail, gb_tessellator_active_region_ref_t region)
{
    ...
    tb_size_t itor = tb_rfind(impl->active_regions, tb_iterator_head(impl->active_regions), tail, region);
...
tb_bool_t gb_tessellator_active_regions_make(gb_tessellator_impl_t* impl)
{
    ...
    {
        // make active region func
        tb_item_func_t func = tb_item_func_mem(sizeof(gb_tessellator_active_region_t), tb_null, tb_null);

        // init the comparator 
        func.comp = gb_tessellator_active_region_comp;

#ifdef __gb_debug__
        // init the c-string function for tb_list_dump
        func.cstr = gb_tessellator_active_region_cstr;
#endif

        // make active regions
        impl->active_regions = tb_list_init(0, func);
    }
